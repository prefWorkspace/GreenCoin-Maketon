...
export default function DropDownIdentify({checkIdentify,setCheckIdentify}){
  const [dropDown,setDropDown] = useState(false);
  return (
    <View style={styles.container}>
      <View style={styles.itemContainer}>      
        <View style={styles.row}>
          <TouchableOpacity  style={styles.checkContainer} activeOpacity={1} onPress={()=>{setCheckIdentify(!checkIdentify)}}>
            {
              checkIdentify?
          ...
          <Text style={styles.title}>본인 확인 서비스 전체동의</Text>
          {
            dropDown ? 
